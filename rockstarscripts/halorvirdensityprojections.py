...
for index in rockstar.snapshot_index:
    ...
    if position == [] or len(position) > 1:
        ...
        halo_data = rockstar.halo_data_largest[index][1]
        ...
        x = float(largest_halo[8])/domain_width
        y = float(largest_halo[9])/domain_width
        z = float(largest_halo[10])/domain_width
        rvir = float(largest_halo[4])
        center = [x, y, z]
